...
bool Process_Expression(string expression) {
  try {
    ...
    for (int count = 0; count < length; count++) {
      ...
      if (count == 0 &&
          (expression[count] == O_BRACKET || IsNumber(expression[count]))) {
        continue;
      }
      if (expression[count] == DECIMAL &&
          (expression[count - 1] == 32 || expression[count + 1] == 32)) {
        throw "Please re-enter the expression with appropriate spacing.";
      }
      if (IsNumber(expression[count])) {
        for (int i = count + 1; i < length; i++) {
          if (expression[i] == DECIMAL ||
              IsOperator(expression[i] || IsNumber(expression[i]))) {
            break;
