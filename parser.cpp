...
bool IsUnary(string expression, int count) {
  try {
    if (count == 0 &&
        (expression[count] == PLUS || expression[count] == MINUS)) {
      return true;
    } else if (count == 0) {
      return false;
    }
    if ((IsNumber(expression[count - 1]) ||
         expression[count - 1] == C_BRACKET) &&
        IsOperator(expression[count]) &&
        (expression[count + 1] == PLUS || expression[count + 1] == MINUS)) {
      return false;
    }
    if (IsOperator(expression[count - 1]) &&
        (expression[count] == PLUS || expression[count] == MINUS)) {
      return true;
    }
    ...
    cout << "An error occurred, please ensure you've written the expression "
            "correctly"
         << endl;
  }
  return false;
...
float Parse_Expression(string expression, int length) {
  ...
  for (int count = 0; count < length; ++count) {
    ...
    if (curr_char == O_BRACKET) {
      ...
      deque.push_back(Parse_Expression(expression_in_brackets,
                                       expression_in_brackets.length()));
    }
